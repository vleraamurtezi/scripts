...
# Map RoBERTa sentiment labels to numeric values
sentiment_mapping = {'Positive': 1, 'Neutral': 0, 'Negative': -1}
df['roberta_sentiment_num'] = df['roberta_sentiment'].map(sentiment_mapping)

# List of topics to include as predictors, excluding 'topic_categorical'
...
# Run the linear model for each genre with RoBERTa sentiment
for genre in df['Genre'].unique():
    # Filter the data for the current genre
    genre_df = df[df['Genre'] == genre]

    # Set RoBERTa sentiment as the dependent variable
    y = genre_df['roberta_sentiment_num']  # Using RoBERTa sentiment now
...
# Collect coefficients and errors for RoBERTa sentiment
for genre in df['Genre'].unique():
    genre_df = df[df['Genre'] == genre]
    y = genre_df['roberta_sentiment_num']
    X = genre_df[topic_columns]
    X = sm.add_constant(X)
    model = sm.OLS(y, X).fit()
    ...
    for topic in topic_columns:
        ...
        genres.append(genre)
        # Rename topics based on mapping
...
# Title and labels
#plt.title("Coefficient Estimates for Topics Across Genres (RoBERTa Sentiment) with Confidence Intervals")
plt.xlabel("Topics")
